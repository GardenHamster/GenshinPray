...
namespace GenshinPray.Service.PrayService
{
    public abstract class BasePrayService : BaseService
    {
        ...
        {
            var sortList = YSPrayRecords.OrderBy(c => c.GoodsItem.GoodsType).ThenByDescending(c => c.GoodsItem.RareType);//先按物品种类排序（0->2），相同种类的物品之间按稀有度倒序排序（5->1）
            return sortList.ToArray();
        }


        /// <summary>
        /// 获取{maxSurplus}发内,成员获得5星角色的累计祈愿次数,0代表还未获得S
        /// </summary>
        /// <param name="YSPrayRecords"></param>
        /// <param name="floorSurplus">当前低保数</param>
        /// <param name="maxSurplus">最大保底数</param>
        /// <returns></returns>
        public int GetStar5Cost(YSPrayRecord[] YSPrayRecords, int floorSurplus, int maxSurplus = 90)
        {
            int star5Index = -1;
